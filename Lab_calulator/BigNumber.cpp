...
namespace bgn {
	...
	static string add_positive_number(string str1, string str2) {
		...
		bool floatHighCarry = true;
		int ints1 = integer1.size();
		int ints2 = integer2.size();
		int fs1, fs2;
		string numb = "0123456789";
		string floatRes;
		string integerRes = add_unsigned_number(integer1, integer2);
		if (float1.size() == 0 && float2.size() == 0 &&
			(integer1[ints1 - 1] == numb[0] || integer1[ints1 - 1] == numb[1] || integer1[ints1 - 1] == numb[2]
				|| integer1[ints1 - 1] == numb[3] || integer1[ints1 - 1] == numb[4] || integer1[ints1 - 1] == numb[5]
				|| integer1[ints1 - 1] == numb[6] || integer1[ints1 - 1] == numb[7] || integer1[ints1 - 1] == numb[8]
				|| integer1[ints1 - 1] == numb[9])
			|| (integer2[ints2 - 1] == numb[0] || integer2[ints2 - 1] == numb[1] || integer2[ints2 - 1] == numb[2]
				|| integer2[ints2 - 1] == numb[3] || integer2[ints2 - 1] == numb[4] || integer2[ints2 - 1] == numb[5]
				|| integer2[ints2 - 1] == numb[6] || integer2[ints2 - 1] == numb[7] || integer2[ints2 - 1] == numb[8])
			|| integer2[ints2 - 1] == numb[9]) {
			floatHighCarry = false;
		}
		if (float1.size() > 0 || float2.size() > 0) {
			fs1 = float1.size(), fs2 = float2.size();
			int max = Maxim(fs1, fs2);
			floatRes = add_positive_number_of_float(float1, float2, floatHighCarry);
			if (floatRes.size() == (max + 1)) {
				floatRes = floatRes.substr(1, floatRes.size() - 1);
				floatHighCarry = true;
			}
		}
		if (floatHighCarry) {
	...
	static string sub_same_size_positive_number_great_small(string str1, string str2) {
		...
		if (i > 0) {
			if (str1.size() == i || isFloat) { i = i - 1; }
			str1.erase(0,/* (str1.size() == i || isFloat) ? i - 1 :*/ i);
		}
	...
	bool isGreaterPositive(string str1, string str2) {
		string float1, float2;
		string integer1 = str1, integer2 = str2;
		// get integer string
		size_t found1 = str1.find('.');
		...
		double int1 = std::stod(integer1.c_str());
		double int2 = std::stod(integer2.c_str());
		double fl1 = std::stod(float1.c_str());
		double fl2 = std::stod(float2.c_str());
		if (int1 > int2) {
			return true;
		}
		else if (int1 < int2) {
	...
	bool isGreater(string str1, string str2) {
		...
		if (str1[0] != '-' && str2[0] == '-') { //+ -
			...
			return isGreaterPositive(str1, str2);
		}
		else { //- -
			str1 = str1.substr(1, str1.size() - 1);
			str2 = str2.substr(1, str2.size() - 1);
			return isGreaterPositive(str2, str1);
		}


	}
	bool isEqualPositive(string str1, string str2) {
		string float1, float2;
		string integer1 = str1, integer2 = str2;
		// get integer string
		size_t found1 = str1.find('.');
	...
	bool isEqual(string str1, string str2) {
		...
		if (str1[0] != '-' && str2[0] == '-') { //+ -
			...
			return isEqualPositive(str1, str2);
		}
		else { //- -
			str1 = str1.substr(1, str1.size() - 1);
			str2 = str2.substr(1, str2.size() - 1);
			return isEqualPositive(str2, str1);
		}
	...
	string operator - (const bgn::BigNumber& a, const bgn::BigNumber& b) {
	...
	bool operator>(const BigNumber& a, const BigNumber& b) {
		return isGreater(a.getValue(), b.getValue());
	}
